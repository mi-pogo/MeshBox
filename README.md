...
For reading the GPIO pins associated with encoders and buttons MeshButtons uses memory mapped registers, via dev/mem and mmap, for faster acces while for sending commands to Kodi, MeshButtons uses Kodi Event Server. The latter one requires Kodi Event Client library to be installed on the system where MeshButtons is compiled.
